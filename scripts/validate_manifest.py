...
def main() -> int:
    ...
    for index, asset in enumerate(manifest["assets"], start=1):
        ...
        required = ["source", "kind", "name", "path", "sha256", "license", "author", "source_url", "extracted_date"]
        ...
        if actual != expected:
            failures.append(f"asset #{index}: sha256 mismatch for {asset['path']}: expected {expected}, got {actual}")

        provenance_path = ROOT / str(asset["source"]) / "provenance.yaml"
        if not provenance_path.is_file():
            failures.append(f"asset #{index}: missing provenance file {provenance_path.relative_to(ROOT)}")
